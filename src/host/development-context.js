...
export function buildDevelopmentPrompt({ targets, targetSourceScope, targetFieldSubset = {}, sources }) {
  ...
  return [
    'NPC State Alpha Development v1. Return one JSON object only; no markdown/commentary. Decide directly from supplied evidence; do not narrate or deliberate about schema choices.',
    'Evidence: cite only target.sourceScope from SOURCES as source={"sourceRef":"...","excerpt":"<verbatim substring>"}. Never use sourceId. Every source excerpt must directly entail the value it supports; do not cite a weaker nearby sentence for a more specific claim.',
    ...
    'Durability: explicit durable facts may establish immediately. A single mood, reaction, sleep state, gesture, wording choice, or action MUST NOT establish personality, behavioralProfile, speech, or mannerisms. Prefer reviewed_no_proposals (or a tentative observation when genuinely useful) over generalizing one scene into a permanent trait. Only explicit characterization or reinforced/repeated patterns may establish/refine durable behavior. Contradictions qualify the affected value, not unrelated accepted detail. Omit unchanged/unsupported fields; omission preserves; never invent.',
    ...
    'Proposal is {targetId,<facet>:...}, never {field,value}. Shapes: relationshipDynamic/canonicalAppearance/behavioralProfile/speech={value,source}; personality={traits:[...],source}; mannerisms={items:[...],source}; facts={role?,species?,background?,actualAge?,apparentAge?,birthday?,source} with string fact values. The one facts.source must directly support EVERY fact included in that facts object; if one excerpt cannot support all candidate facts, emit only the facts it directly supports and omit the rest rather than stretching the citation. Collections: appearanceForms={operation,forms:[...]}; importantMemories={operation,memories:[...]}; nonPlayerRelationships={operation,relationships:[{targetId?|targetRef?|targetName?,relationship|relationKind|description,source}]}. New collection entries use add; replace/remove require the stable persisted formId/memoryId/relationId supplied in TARGET.',
