...
export function buildDevelopmentPrompt({ targets, targetSourceScope, targetFieldSubset = {}, sources }) {
  ...
  return [
    'NPC State Alpha Development v1. Return one JSON object; no markdown/commentary.',
    'Evidence: source={"sourceRef":"...","excerpt":"<verbatim substring>"} from target.sourceScope/SOURCES; never sourceId. Excerpt must support value.',
    ...
    'Durability: explicit durable facts may establish immediately. One-off/transient mood/reaction/sleep/gesture/wording/action is NOT personality, behavioralProfile, speech or mannerisms; use reviewed_no_proposals/tentative observation. Only explicit characterization or reinforced/repeated patterns establish/refine. Contradictions qualify affected value. Omit unchanged/unsupported; omission preserves; never invent.',
    ...
    'Proposal={targetId,<facet>:...}, never {field,value}. Scalar shapes: relationshipDynamic/canonicalAppearance/behavioralProfile/speech={value,source}; personality={traits:[...],source}; mannerisms={items:[...],source}; facts={role?,species?,background?,actualAge?,apparentAge?,birthday?,source}; its source supports every included fact, else split/omit. Collections: appearanceForms={operation,forms:[...]}; importantMemories={operation,memories:[...]}; nonPlayerRelationships={operation,relationships:[{targetId?|targetRef?|targetName?,relationship|relationKind|description,source}]}. New:add. replace/remove require IDs supplied in TARGET.',
