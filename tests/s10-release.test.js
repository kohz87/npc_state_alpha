...
test('S10 release version owners remain 0.1.0 while state/native schemas stay version 1', () => {
...
test('S10 package is deterministic and excludes development/private content', () => {
  withTempPackage((first, tempRoot) => {
    ...
    assert.equal(checksum, `${second.archiveSha256}  npc_state_alpha-0.1.0.zip`);
  });
});
