...
## S10 final release/publication handoff — 2026-09-11

S10 is implemented only in workspace `182464a1-s10-release`, branch `work/182464a1-s10-release`, directly from accepted S9 `e19e0a40ec35a772684bdc7e1ad555a803776a1e`. Runtime architecture remains the accepted S1-S9 system; S10 adds release hardening, deterministic packaging/CI/docs and one narrow evidence-backed legacy adapter.

Release version remains **0.1.0** in both `package.json` and `manifest.json`. Canonical state remains `npc_state_alpha.v1` schema 1 and native portability remains `npc_state_alpha.native_state` format version 1, so S9 -> S10 requires no Alpha persistence-schema migration. The representative upgrade regression preserves stable IDs, live/durable fields, relationship axes, corrections/locks, pending review, checkpoints and canonical `developmentConnectionProfile` settings.

`npm run package` now builds both the direct-install `dist/` tree and deterministic `release/npc_state_alpha-0.1.0.zip` plus SHA-256 sidecar. Two consecutive canonical builds matched exactly at **40 runtime files / 832,317 bytes** with ZIP SHA-256 **`1b66506a5ab534049286ac2f82b464b5398034263cc8bb77bf8906fbb25f307c`**. Release content excludes tests, docs, scripts, node_modules, Git metadata, logs, credentials and SillyTavern user data. The earlier pristine-checkout package-test dependency and a parallel shared-`dist` test race were both found and fixed by self-contained isolated package builds.

Clean-install evidence is deterministic because the current controller exposes only one fixed SillyTavern Alpha data root, not a safe second disposable user/profile creator. S10 builds into an isolated temporary package root and initializes the packaged production adapter in a fresh empty host fixture, proving empty state, loader boundary, Immediate interceptor/prompt integration and listener cleanup. This is deliberately not overstated as a second real SillyTavern profile.

The real dedicated host was used for the actual **S9 -> S10 upgrade**. Before sync, the accepted S9 host passed Edge acceptance and the S10 legacy module URL returned HTTP 404. After stopping the controller-owned host, synchronizing only S10 `dist`, and restarting, the artifact is **40 files / 832,317 bytes** and `src/state/legacy-beta-v044.js` serves JavaScript HTTP 200. Installed manifest is 0.1.0. SillyTavern **1.18.0** at commit `8172dcd0ee672d3cd9a5e5f7af134f91a45cd2b8` passes real Edge loader/module/adapter/interceptor and Dossiers/Settings/Development/lock/relationship/chat-switch/listener/reopen acceptance with **0 Alpha console errors / 0 uncaught page errors**; host stderr is empty.

Legacy compatibility is exact and source-backed. Read-only public Beta inspection established the supported tuple: NPC State Beta **0.4.44**, commit **`a34f5f27385b6fba75b8ff5832015ba66ea4d0c2`**, canonical `v03/schema.js` schema 1 and `v03/bundle.js` format `npc_state_v3_bundle` version 1. `src/state/legacy-beta-v044.js` is Alpha-owned clean-room translation code with no Beta runtime dependency. It imports only safely representable current values and stable-ID social/family edges; legacy relationship/evidence/history/provenance/manual markers/unsupported portrait/deletion/suppression/name-only data are reported as omitted rather than fabricated. Synthetic compatibility fixtures are conformance objects derived from authoritative schema/serializer code and are not claimed as independent real-export evidence.

Beta **0.5.38 is explicitly unsupported**. At inspected Beta commit `044cd42effa5c80a2e5f0facc6deeffa106ade01`, canonical bundle creation identifies app 0.5.38 while the corresponding parser accepts only 0.3.x/0.4.x for the v3-compatible bundle. Alpha therefore rejects 0.5.38 rather than guessing or repairing the legacy schema.

Legacy installation requires an empty Alpha state, explicit `IMPORT_BETA_V044_INTO_EMPTY_ALPHA` confirmation, and either a verified canonical host history boundary or `emptyHostHistory=true` for a genuinely empty chat. Installation uses Alpha CAS, never mutates the source bundle/database/profile, never merges by name and creates the existing `import_baseline`. A production history regression proves changed pre-import history is blocked as `pre_import_history_untrusted`. Successful legacy state is reloadable and can be exported through the unchanged native Alpha format.

Repository release workflow is now documented in `README.md` and enforced by one `.github/workflows/ci.yml`: Node 22, `npm test`, `npm run validate`, `npm run measure:prompts`, `npm run measure:performance`, `npm run package`, `git diff --check`, then release ZIP/checksum artifact upload. CI uses no model/provider/SillyTavern/Beta user credentials. S7 prompt hashes remain exact at **16,073 Immediate / 19,053 Development aggregate chars**. The S9 performance harness passes unchanged on S10, with checkpoint cap 32, bounded prompts and valid long/integrated state; the final pre-documentation run measured the 128-exchange local medians at ~43 ms pre-provider / ~162 ms post-provider and 112-exchange LARGE old reconstruction at ~1.17 s, reported only as machine-dependent characterization.

Current deterministic pre-final-gate result is **573/573 PASS**, S10 validation **0 errors**, prompt freeze PASS, performance harness PASS and real installed-host upgrade PASS. Final post-documentation tests/validation/prompt/performance/package/diff-check, complete S9 -> S10 adversarial review, exact-package host recheck and the single local S10 commit still remain before publication.

Controller-only infrastructure note: v0.10.17 added one guarded isolated-worktree `directory_create` operation after full staged security validation because S10 needed `.github/workflows`. The current chat MCP schema cache did not expose the new function after restart, so the directory was created through one reversible package-hook run that was immediately restored. No generic shell was added. Codex registration was merely verified; Codex was not invoked.

Publication has **not** occurred at this handoff point. Canonical local `main` remains untouched. After final local acceptance and the single S10 commit, perform a fresh GitHub remote-main read, verify the configured origin/ancestry/fast-forward conditions, then publish the exact accepted commit only by a normal non-force push. If remote main contains non-ancestor work, stop without force/rebase/merge. No Opus or Gemini was consumed for S10, and no production Beta data was accessed.
